...
def standard_ref(text):
    ...
    impossible_pattern_ending = u'(?!(</a>)|\d)'
    regex = re.compile('(' + possible_named_reference + numbered_reference +
                       possible_pilcrow_reference + ')')
    ...
    for elem in matched:
        ...
        text = re.sub(
            re.escape(full_paragraph) + impossible_pattern_ending,
